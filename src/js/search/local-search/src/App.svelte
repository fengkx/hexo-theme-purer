...
<div>
	<div class="search-header bg-gray-400">
		<input id="actual-search-input" model="keyword" ref="input" class="inline-block w-full h-10 px-2 py-1" placeholder="{placeholder}" type="text">
	</div>
	<div class="search-result bg-gray-200">
		...
		<a href="/{ post.path }" class="result-item block px-2 pb-3 mb-1 pt-1 hover:bg-indigo-100">
			...
			<p class="result-content text-gray-600 text-sm">{ post.text }</p>
		</a>
		{/each}
	</div>
</div>
